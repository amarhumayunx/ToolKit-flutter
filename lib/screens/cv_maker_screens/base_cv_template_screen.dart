...
class _BaseCVTemplateScreenState extends State<BaseCVTemplateScreen> {
  Future<Map<String, dynamic>> _collectAllFormData() async {
    ...
    return {
      ...
      'websites': websites, // Add websites data here
      'templateId': templateProvider.selectedTemplateId,
    };
  }

// Updated _exportAsPdf method in BaseCVTemplateScreen
  // Updated _exportAsPdf method in BaseCVTemplateScreen
  Future<void> _exportAsPdf({required bool openAfterExport}) async {
    try {
      ...
      if (mounted) {
        ...
        // Clear data and navigate - but don't do it twice!
        _clearAllData();
      }
    } catch (e) {
      ...
      rethrow; // Re-throw so _handleExportAction can catch it
    }
  }
